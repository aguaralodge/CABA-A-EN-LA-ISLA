...
if(ingresoEl && egresoEl && personasEl && payBtn && outNoches && outTotal && outSaldo){
  ...
  function addRange(checkin, checkout){
    ...
    // ocupa desde checkin inclusive hasta el día anterior a checkout
  ...
  // Flatpickr: deshabilita y pinta en gris las fechas ocupadas
  function initPickers(){
    if(!window.flatpickr) return; // fallback: inputs normales
    const common = {
      dateFormat: "Y-m-d",
      disableMobile: true,
      disable: [
  ...
  const SENIA = 25000;
  // Cargamos fechas ocupadas y luego inicializamos calendario
